...
#---------------------------#
Q2DTor = "/home/david/Cathedral/q2dtor.py"
...
def option_createINPs(FOLDER,FREQSCAL,LEVEL):
    ...
    print()
#---------------------------------#
def option_init(FOLDER,CASE):
    # Geometries
    dict_SXX = get_geometries()
    systems  = sorted(dict_SXX.keys())
    # PWD and nohup file
    PWD = os.getcwd()+"/"
    # Go system by system
    Q2DTor = input("     Path to Q2DTor executable: ").strip()
    if not os.path.exists(Q2DTor):
    ...
    for SXX in systems:
        ...
        command = "python3.6 %s %s --print --init"%(Q2DTor,SXX)
...
#=================================#
def option_checksp(FOLDER,CASE):
    PWD = os.getcwd()+"/"
    # Geometries
    dict_SXX = get_geometries()
    systems  = sorted(dict_SXX.keys())
    ...
    for SXX in systems:
        ...
        strnsp = "        %3s: num(SP) = %2i (%2i YES, %2i NO) vs %2i "%(SXX,nsp,nYES,nNO,NSP)
...
def option_execute(FOLDER,CASE):
    ...
    string += "        (5) --optsp\n"
    string += "        (6) --tor2dns\n"
    string += "        (7) --rovibpf thermo\n"
    string += "        (8) --pdf\n"
    string += "        --------------------------\n"
    string += "        use .. to exit\n"
    string += "        --------------------------\n"
    string += "        type option and system to only\n"
    string += "        apply to one of them. Example:\n"
    string += "          >> 2 S01                    \n"
    string += "        --------------------------\n"
    print(string)
    # Geometries
    ...
    while True:
       for SXX in systems:
           ...
           #----------------#
           if answer in "567":
               case1 = (answer == "5"  and NSP != nsp)
               case2 = (answer in "67" and (nNO !=  0 ))
               case3 = (answer in "67" and (NSP != nsp))
               if case1 or case2 or case3: print(strnsp); continue
           #----------------#
           # execute Q2DTor #
           #----------------#
...
def max_rel_diff(v1,v2):
    if len(v2) == 0: return -2
    if len(v1) != len(v2): return -1
    mdiff  = 100*max([abs(v2-v1)/v1 for v1,v2 in zip(v1,v2)])
    return mdiff
#---------------------------------#
def option_checkdata(FOLDER,root,CASE):
    EPS_REL = 2.5
    EPS_ABS = 0.25
    blank   = "      "
    # Geometries
    ...
    if root:
       ...
       if CASE == "1": reffile = "RefDataQ2DTor_GAUSSIAN.txt"
       if CASE == "2": reffile = "RefDataQ2DTor_ORCA.txt"
    ...
    for SXX in sorted(dict_SXX.keys()):
        ...
        if not os.path.exists(outfile):
           print("File does not exist: %s"%outfile)
           continue
        Tlist, data = data_from_q2dtor_out(outfile)
        # Compare temperatures
        maxdiff = max_rel_diff(Tlist_ref,Tlist)
        if   maxdiff == -2:
             fstring += "       temperatures not found!       FAIL\n"
             fstring += blank+"=============================\n"
             print(fstring)
             continue
        elif maxdiff == -1:
             fstring += "       num. of temperatures differs! FAIL\n"
             fstring += blank+"=============================\n"
             print(fstring)
             continue
        elif maxdiff > 1.0:
             fstring += "       temperatures differ in value! FAIL\n"
             fstring += blank+"=============================\n"
             print(fstring)
             continue
        # common data
        ...
        for variable in variables:
            ...
            if variable[0] in  ["U","H","G","S","C"]:
                ...
                if mdiff == -1: string += blank+" Number of %s differs!\n"%variable
                if mdiff > EPS_ABS: string += blank+" Abs.Err.(%-9s): %6.1f  WRONG\n"%(variable,mdiff)
            else:
                mdiff = max_rel_diff(vec1,vec2)
               #if mdiff == -1: exit("Number of %s differs!"%variable)
                if mdiff == -1: string += blank + " Number of %s differs!\n"%variable
                if mdiff > EPS_REL: string += blank+" Rel.Err.(%-9s): %6.1f%% WRONG\n"%(variable,mdiff)
        # Print data
        if string != "":
           fstring += string
        else:
           fstring += blank+" Rel.Err.(pfns) < %.1f%%        OK\n"%EPS_REL
           fstring += blank+" Abs.Err.(ther) < %.2f units  OK\n"%EPS_ABS
        fstring += blank+"=============================\n"
        print(fstring)
...
def option_copyfrombackup(FOLDER,BKFOLDER):
    ...
    string  = "\n"
    string += "        Option?\n"
    string += "        (a) copy PES from backup\n"
    string += "        (b) copy VFIT from backup\n"
    string += "        (c) copy SPLIST from backup\n"
    string += "        (d) copy GTSs from backup\n"
    string += "        (e) copy DFIT from backup\n"
    string += "        (f) copy EVALS from backup\n"
    string += "        (g) copy SQRT from backup\n"
    string += "        --------------------------\n"
    string += "        use .. to exit\n"
    string += "        --------------------------\n"
    string += "        type option and system to only\n"
...
def get_case(root):
    ...
    nn  = 2
    # Select case
    string  = ibs+"Select case:\n"
    string += ibs+" (1) HF/sto-3g (GAUSSIAN)\n"
    string += ibs+" (2) HF/sto-3g (ORCA)\n"
    if root:
       string += ibs+" (3) MPWB1K (GAUSSIAN)\n"
       nn = 3
    string += ibs+" ..  to exit\n"
    print(string+"\n")
    CASE = input(ibs+"your choice: ").strip()
    print()
    if CASE not in [str(i+1) for i in range(nn)]: return -1
    return CASE
#-------------------------------------------#
def get_action(root):
    ibs = "    "
    # Select action
    string  = ibs+"Select action:\n"
    string += ibs+" (1) Create input files\n"
    string += ibs+" (2) Check number of Stationary Points\n"
    string += ibs+" (3) Check results against refdata file\n"
    if root:
       string += ibs+" (4) Execute Q2DTor\n"
       string += ibs+" (5) Generate reference-data file\n"
       string += ibs+" (6) Backup data\n"
       string += ibs+" (7) Copy data from backup\n"
       nn = 7
    else: nn = 3
    string += ibs+" ..  to exit\n"
    print(string+"\n")
    ...
    print()
    if ACTION not in [str(i+1) for i in range(nn)]: return -1
    return ACTION
#-------------------------------------------#
...
def main(root=False):
    ...
    while True:
      ...
      while True:
         ...
         if   ACTION ==  "1": option_createINPs(FOLDER,FREQSCAL,LEVEL) #; option_init(FOLDER,CASE)
         elif ACTION ==  "2": option_checksp(FOLDER,CASE)
         elif ACTION ==  "3": option_checkdata(FOLDER,root,CASE)
         elif ACTION ==  "4": option_execute(FOLDER,CASE)
         elif ACTION ==  "5": option_gendata(FOLDER)
         elif ACTION ==  "6": option_backup(FOLDER,BKFOLDER)
         elif ACTION ==  "7": option_copyfrombackup(FOLDER,BKFOLDER)
         else               : break
         print()
#===========================================#
